...
def compileOccupancy(directories, map_size, resolution, show_plot=False):
    ...
    for i in range(len(directories)):
        ...
        if (show_plot is True):
            axarr[i//n_half][i%n_half].set_title(f"m{i}")
    ...
    if (show_plot is True):
        ...
        plt.gca().invert_yaxis()
        plt.title("Composite Occupancy Map", fontsize=16)
        ...
        plt.savefig("composite.svg", format='svg')
        plt.show()
...
def objects_seen_by_waypoint(auv_pose, object_bboxes, sonar_range):
    auv_x, auv_y, auv_z, auv_yaw = map(float, auv_pose) 
    detected_objects = []

    perpendicular_direction = np.array([
        np.cos(np.radians(auv_yaw)),  
        -np.sin(np.radians(auv_yaw))  
    ], dtype=np.float64)
...
def compute_sidescan_coverage(waypoints, occupancy_grid, sonar_range, resolution):
    ...
    for (wx, wy, wz, yaw) in waypoints:
        gx, gy = wx, wy  
        sonar_width = int(sonar_range / resolution)  
